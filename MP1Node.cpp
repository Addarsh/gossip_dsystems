...
void MP1Node::updateMemberShip (char *data, int size){  
  ...
  for(int i = 0; i < numelems; i++){
    for (myit = memberNode->memberList.begin(); 
            myit != memberNode->memberList.end (); myit++){
      if ((myit->getid() == mentry[i].getid()) && 
              (myit->getport() == mentry[i].getport())){
        if ((myit->getheartbeat() != -1) && 
              (mentry[i].getheartbeat() > myit->getheartbeat())){ 
